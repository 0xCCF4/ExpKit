...
def _parse_source(match: re.Match) -> str:
    global __status
    assert PARSE_LOCK.locked(), "Parse lock is not locked"
    ...
    if __status == STATUS_NORMAL:  # no string
        if at_sign:
            __status = STATUS_AT_STRING  # @ string
            token = "{{BEGIN_AT_STRING}}"
            result = token + postfix
        else:
            __status = STATUS_STRING  # normal string
            token = "{{BEGIN_NA_STRING}}"
            result = token + postfix
    elif __status == STATUS_AT_STRING:  # @ string
        if double_quote:
    ...
    elif __status == STATUS_STRING:  # normal string
        if escaped_quote:
...
def _replace_strings_normal(match: re.Match) -> str:
...
class CSharpStringTransformTemplate(StageTaskTemplate):
    def __init__(self):
        super(CSharpStringTransformTemplate, self).__init__(
    ...
    def execute(self, parameters: dict) -> bool:
        super(CSharpStringTransformTemplate, self).execute(parameters)

        for origin, target in parameters["files"]:
            ...
            target_source = self.__transform(origin_source, transform)

            target_path.write_text(target_source, "utf-8")
            return True


    def __transform(self, source: str, transform: Callable[[re.Match], str]) -> str:
        with PARSE_LOCK:
            __status = STATUS_NORMAL
            ...
            while old_source != source:
                old_source = source
                source = cmp.sub(_parse_source, source, count=1)
            ...
            source = cmp.sub(_replace_strings_normal, source)
